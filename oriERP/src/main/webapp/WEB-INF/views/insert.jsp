    ...
    //비용 금액 입력시
      ...
      $("#deposittbl").on("click",".drow",function(){
         ...
         $("#deposittbl1").on("keyup",".amount",function(){
            ...
            var balance=parseInt(uncomma($(this).parent("td").find(".balance").attr("balance")));
//             alert($(this).parent().parent("tr").find(".balance").html());
            ...
            if(type=='입금'){
               ...
               $(this).parent().parent("tr").next().find(".sum").val(sum);
               
            }
            if(type=='출금'){
               uncomma($(this).parent().parent().find(".sum").val(minus));
               $(this).parent().parent("tr").next().find(".balance").find("input").val(minus);
               $(this).parent().parent("tr").next().find(".sum").val(minus);
               
               if(strminus<0){
                  alert("잔액보다 큰 금액은 입력할 수 없습니다.");
                  $(this).val("");
                  $(this).parent().parent().find(".sum").val(balance).toLocaleString();
                  $(this).focus();
            ...
            $("#deposittbl1").find("input[name='"+attcode+"']").change(function(){
               if(this.value=='입금') {
                  ...
                  console.log(minus);
                  uncomma($(this).parent().parent().find(".sum").val(minus));
                  $(this).parent().parent("ttr").next().find(".balance").find("input").val(minus);
                  $(this).parent().parent("tr").next().find(".sum").val(minus);
